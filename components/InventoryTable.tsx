...
export default function InventoryTable({ items, onEdit, onDelete, onUpdateArrived }: InventoryTableProps) {
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                  {['Name', 'Category', 'Unit', 'Needed', 'Arrived', 'Status', 'Assigned To', 'Actions'].map(h => (
                    <th key={h} className="px-4 py-3 text-left font-medium text-xs uppercase tracking-wide" style={{ color: '#8b95aa' }}>
                      {h}
                    </th>
                  ))}
                </tr>
    ...
    >
      <svg width="80" height="80" viewBox="0 0 100 100" className="mb-4 opacity-30">
        ...
        {[0, 60, 120, 180, 240, 300].map(angle => {
          ...
          return <line key={angle} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#FF7518" strokeWidth="1.5" />;
        })}
      </svg>
