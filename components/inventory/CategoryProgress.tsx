...
export default function CategoryProgress({ items }: { items: InventoryItem[] }) {
  ...
  return (
    <div className="glass p-5">
      <h2 className="font-semibold text-base mb-4" style={{ color: '#f0f4ff' }}>Category Progress</h2>
      <div className="flex flex-col gap-4">
        {stats.map(({ cat, arrived, needed, pct }, i) => (
          <div key={cat}>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-sm font-medium" style={{ color: '#f0f4ff' }}>{cat}</span>
              <span className="text-xs" style={{ color: '#8b95aa' }}>{arrived}/{needed} units · {pct}%</span>
            </div>
